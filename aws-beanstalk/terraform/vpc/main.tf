...
resource "aws_nat_gateway" "main" {
  ...
  tags = {
    Name        = "${var.application_name}-nat-${var.environment}-${format("%03d", count.index+1)}"
    Environment = var.environment
  }
}
...
resource "aws_eip" "nat" {
  ...
  tags = {
    Name        = "${var.application_name}-eip-${var.environment}-${format("%03d", count.index+1)}"
    Environment = var.environment
  }
}
...
resource "aws_cloudwatch_log_group" "main" {
  name = "${var.application_name}-${var.environment}-cloudwatch-log-group"
}

resource "aws_iam_role" "vpc-flow-logs-role" {
  name = "${var.application_name}-vpc-flow-logs-role"
...
resource "aws_iam_role_policy" "vpc-flow-logs-policy" {
  name = "${var.application_name}-vpc-flow-logs-policy"
